...
public class CustomCsrfTokenRepository implements CsrfTokenRepository {
    ...
    private final Logger logger = LoggerFactory.getLogger(CustomCsrfTokenRepository.class);
    ...
    public void saveToken(CsrfToken token, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (existingToken.isPresent()) {
            ...
            csrfToken.changeToken(csrfToken.getToken());
        } else {
            ...
            logger.warn("토큰 저장 완료 : {}", csrfToken);
        }
    }
    ...
    public CsrfToken loadToken(HttpServletRequest request) {
        logger.error("토큰 로드");
        String identifer = request.getHeader("X-IDENTIFIER");

        Optional<Token> existingToken = jpaTokenRepository.findTokenByIdentifier(identifer);

        if (existingToken.isPresent()) {
            logger.warn("토큰 값 : {}", existingToken.get().getToken());
            Token csrfToken = existingToken.get();
